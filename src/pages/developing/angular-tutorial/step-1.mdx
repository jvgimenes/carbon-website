...
This will create the new project within the current directory, by using `--directory ./`. Make sure you do
...
Before we start adding components we want to start with an empty project, so delete everything in `app.component.html` except for the `router-outlet`. We will also have to delete the test that was associated with this code. So in `app.component.spec.ts`, delete the should render title and should have as title `'carbon-angular-tutorial'` test.
...
Now let's import the icons from our `@carbon/icons` package. In `app.module.ts`, we need to import each
...
Also make sure to update all the necessary imports for `IconService`.
...
As the application created is strict by default, it is necessary to create a file, inside src folder called typings.d.ts with the declaration of carbon icons, otherwise we may receive some type errors.
...
We have to delete all the content into `app.component.html` and add the following HTML:
...
The `@HostBinding` tag need to be added right above the constructor method.
...
ng g component home/landing-page
```
...
```bash
ng g module repositories --routing
ng g component repositories/repo-pagefix
```
...
We need to update the routing functionality to enable the loading of the `repositories` component.
...
```
For the following files, add the route:

For the `home-routing.module.ts`:
...
```
For the `repositories-routing.module.ts`:
<!-- prettier-ignore-start -->
...
We need to overwrite the old router into `header.component.html` and add the new one.
...
And add into the same file:

```html path=src/app/header/header.component.html
<ibm-header-item [route]="['/repos']">Repositories</ibm-header-item>
```

And to add a route to redirect directly to home page we can remove:
...
```html path=src/app/header/header.component.html
<ibm-header name="Carbon Tutorial Angular" [route]="['']">
```
...
We need to use an updated version of continuos integration file used in `CircleCI`.

Substitute the circleci configuration file for the following code:

```yaml path=.circleci/config.yml
version: 2.1
orbs:
...
You will need to change the script object inside `package.json` file to execute the tests correctly, so the CI can have the correct parameters to be called.
...
```
#### Running tests on you environment.

We have `test` script defined in `package.json`. You'd
typically have that script run your test suite as part of your CI build. Go
...
```bash
git add --all
git commit -m "feat(tutorial): complete step 1"
```
