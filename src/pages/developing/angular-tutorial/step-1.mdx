...
This will create the new project within the current directory, by using `--directory` and `./` parameters. Make sure you do
...
As the application created is a strict type, It is necessary to create a file, inside `src` folder called `typings.d.rs` with the declaration of carbon icons, otherwise we may receive some type errors.
...
We need to overwrite the old router into header component and add the new one.
...
If you have problems with the test running into CI environment, you can use this steps to fix it. Otherwise you can go ahead to `Running tests on you environment` topic.

First you will have to change to .cirvleci/config.yml to use another container image, and set a environment variable.

```yaml path=.cirvleci/config.yml
version: 2.1
jobs:
  step-one:
    docker:
...
You will need to add `npm run test:once` in your scripts inside `package.json` as well to be executed. And also a new library called `puppeteer` to provide a correct browser controller to CI execute the tests properly.
...
```
After that it is necessary to install the added library, so we need to execute:
```bash
npm install
```
